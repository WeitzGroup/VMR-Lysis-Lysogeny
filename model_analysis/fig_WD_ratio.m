...
if (fp<=0)
  ...
  info.m=1/6;  %hr^-1
  info.a=0.1;
  info.beta=20/(1-info.a*0.1);% burst size, match at N=K/10
  info0=info;
  ...
  for j=1:nruns,
    ...
    for k=1:size(LHSample,1)
        ...
        a = info.a*info.beta*info.phi/info.K;
        b = -info.beta*info.phi;
        c = info.m;
        theory.N=(-b-sqrt(b^2-4*a*c))/(2*a);
        theory.V=(info.r*(1-theory.N/info.K)-info.d)/(info.phi*(1-info.a*theory.N/info.K));
        if (theory.N>0 & theory.V>0 & isreal(theory.N))
          count=count+1;
...
set(tmph,'linewidth',3);
tmpt=text(10^5.6,10^2.8,'Weitz-Dushoff (2008)');
